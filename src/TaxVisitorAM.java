...
class TaxVisitorAM implements Visitor {
    ...
    public double visit(Combustivel combustivelItem) {
        ...
        return Double.parseDouble(df.format((combustivelItem.getPrice() * .0) + combustivelItem.getPrice()));
    }
    // Calculates total price based on this being taxed
    // as a Tabaco item
    ...
    public double visit(Alimento alimentoItem) {
        ...
        return Double.parseDouble(df.format((alimentoItem.getPrice() * .0) + alimentoItem.getPrice()));
    }
