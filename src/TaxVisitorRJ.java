...
class TaxVisitorRJ implements Visitor {
    ...
    public double visit(Combustivel combustivelItem) {
        ...
        return Double.parseDouble(df.format((combustivelItem.getPrice() * .18) + combustivelItem.getPrice()));
    }
    // Calculates total price based on this being taxed
    // as a Tabaco item
    public double visit(Tabaco tabacoItem) {
        System.out.println("Tabaco Item: Price with Tax");
        return Double.parseDouble(df.format((tabacoItem.getPrice() * .37) + tabacoItem.getPrice()));
    }
    // Calculates total price based on this being taxed
    // as a Alimento item
